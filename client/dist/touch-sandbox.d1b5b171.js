...
},{}],"3Pvf2":[function(require,module,exports) {
"use strict";
var HMR_HOST = "0.0.0.0";
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "f966a98fd1b5b171";
...
},{}],"glF5W":[function(require,module,exports) {
...
class MouseUpdater {
    wheelEvent(e) {
        ...
        if (e.deltaY > 0) this.state.scale = {
            x: this.state.scale.x * 1.1,
            y: this.state.scale.y * 1.1
        };
        else this.state.scale = {
            x: this.state.scale.x * 0.9,
            y: this.state.scale.y * 0.9
        };
    ...
    mouseEvent(e) {
        ...
        if (downBefore && this.state.primaryDown && _domain.distanceBetween(this.state.startPan, this.state.mousePosition) > this.DRAG_TOLERANCE) {
            this.state.offset = {
                x: this.state.offset.x - (this.state.mousePosition.x - this.state.startPan.x) / this.state.scale.x,
                y: this.state.offset.y - (this.state.mousePosition.y - this.state.startPan.y) / this.state.scale.y
            };
            this.state.startPan = {
                ...this.state.mousePosition
            };
    ...
    touchEvent(e) {
        ...
        if (singleFingerDown) {
            ...
            if (!downBefore) {
                ...
                if (!this.state.touchClickDisabled) this.state.lastPotentialTouchClick = [
                    performance.now(),
                    {
                        ...this.state.mousePosition
                    }
        ...
        if (multiFingersDown) {
            ...
            if (prevTouchPinchDiff > 0 && this.state.touchPinchDiff != prevTouchPinchDiff) {
                const beforeZoom = screenToWorld(this.state.mousePosition, this.state.offset, this.state.scale);
                if (this.state.touchPinchDiff > prevTouchPinchDiff) this.state.scale = {
                    x: this.state.scale.x * 1.1,
                    y: this.state.scale.y * 1.1
                };
                else this.state.scale = {
        ...
        if (e.touches.length === 0) this.state.touchClickDisabled = false;
    }
    update(time) {
...
function worldToScreen(world, offset, scale) {
    return {
        x: (world.x - offset.x) * scale.x,
        y: (world.y - offset.y) * scale.y
    };
}
function screenToWorld(screen, offset, scale) {
    return {
        x: screen.x / scale.x + offset.x,
        y: screen.y / scale.y + offset.y
    };
}
function frame(tileGraphics) {
    return (time)=>{
        ...
        const greenScreenPos = worldToScreen(greenWorldPos, mouseUpdater.state.offset, mouseUpdater.state.scale);
        tileGraphics.drawInactiveTile(ctx, greenScreenPos, {
            colour: _domain.TileColour.Green,
            shape: _domain.TileShape.One
        }, mouseUpdater.state.scale.x);
        const redWorldPos = {
            x: 300,
            y: 300
        };
        const redScreenPos = worldToScreen(redWorldPos, mouseUpdater.state.offset, mouseUpdater.state.scale);
        tileGraphics.drawInactiveTile(ctx, redScreenPos, {
            colour: _domain.TileColour.Red,
            shape: _domain.TileShape.Two
        }, mouseUpdater.state.scale.x);
        clickPositions.forEach((p)=>{
            const screenPos = worldToScreen(p, mouseUpdater.state.offset, mouseUpdater.state.scale);
            tileGraphics.drawInactiveTile(ctx, screenPos, {
                colour: _domain.TileColour.Yellow,
                shape: _domain.TileShape.Three
            }, mouseUpdater.state.scale.x);
        });
        ctx.restore();
...
_tileGraphics.loadTileGraphics().then((tg)=>{
    requestAnimationFrame(frame(tg));
});

},{"../../shared/Domain":"2iCCP","./graphics/TileGraphics":"jbJGn"}],"2iCCP":[function(require,module,exports) {
...
},{}],"jbJGn":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "loadTileGraphics", ()=>loadTileGraphics
);
...
},{"../../../shared/Domain":"2iCCP","./domain":"5in7n","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"5in7n":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "rectFromElement", ()=>rectFromElement
);
parcelHelpers.export(exports, "rectContains", ()=>rectContains
);
...
function rectContains(rect, position) {
    return position.x >= rect.position.x && position.x <= rect.position.x + rect.width && position.y >= rect.position.y && position.y <= rect.position.y + rect.height;
}
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["kn9T2","3Pvf2","glF5W"], "glF5W", "parcelRequire00f1")

//# sourceMappingURL=touch-sandbox.d1b5b171.js.map
